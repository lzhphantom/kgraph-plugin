  ...
  apply(ctx) {
    ...
    async function runDetail(args) {
      try {
        ...
        return { id: id, path: f.rel, kind: f.kind, head: head, imports: f.imports, externals: f.externals }
      } catch (e) {
        return { error: String(e && e.message || e) }
      }
    }

    function buildCommandText(data) {
      const L = []
      L.push('📊 项目知识图谱: ' + data.root)
      L.push('模块 ' + data.stats.modules + ' | 源文件 ' + data.stats.sourceFiles + ' | 技术栈: ' + (data.techStack || []).join(', '))
      L.push('')
      for (const m of data.modules) {
        L.push('▸ ' + m.name + ' (' + m.kind + ', ' + m.tech + ', ' + m.fileCount + ' 文件)')
        if (m.entryPoints && m.entryPoints.length) L.push('  入口: ' + m.entryPoints[0])
        const kc = Object.keys(m.kindCounts || {})
        if (kc.length) L.push('  分层: ' + kc.map(function (k) { return k + '=' + m.kindCounts[k] }).join(', '))
        L.push('')
      }
      if (data.observations && data.observations.length) {
        L.push('观察:')
        for (let i = 0; i < data.observations.length; i++) L.push((i + 1) + '. ' + data.observations[i])
      }
      return L.join('\n').slice(0, 4000)
    }
    ...
    if (commands !== undefined) {
      ctx.effect(function () {
        return commands.register({
          name: 'kgraph',
          description: '扫描项目目录生成知识图谱, 用法: /kgraph {项目路径}',
          input: { hint: '{项目路径}, 例如 G:/code/xxx' },
          handler: async function (invocation) {
            const path = norm(invocation && invocation.rawInput)
            if (!path) return { kind: 'error', text: '用法: /kgraph {项目路径}, 例如 /kgraph G:/code/my-project' }
            try {
              const result = await runScan({ path: path })
              if (result.error) return { kind: 'error', text: result.error }
              return { kind: 'success', text: buildCommandText(result) }
            } catch (e) {
    ...
    ctx.effect(function () { return harness.handle('scan', runScan) })
    ctx.effect(function () { return harness.handle('detail', runDetail) })
    const tool = harness.defineTool({
      ...
      execute: async function (args) { return runScan(args) }
    })
    ctx.effect(function () { return harness.registerTool(ctx, tool) })
    console.log('kgraph host ready')
  }
}
